...
- **An address I am expected to paste goes alone on its own line.** Triple-click selects a whole line in every terminal, so a line holding one address and nothing else copies in one gesture. Double-click cannot replace it: in Apple Terminal `470.2` a double-click ends at `/` and at `:`, though not at `.`, so it returns one path segment (`dotfiles`), one host (`github.com`) or the bare scheme (`https`), never the whole address. Measured 2026-09-02.
