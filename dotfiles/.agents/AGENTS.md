...
A branch-wide `gh run list --json ...` does not answer "what ran last": it returned runs several weeks old at the top of the list, repeatedly, in the same session where the per-workflow form was correct. Read the newest run through `gh run list --workflow={file} --branch={branch} --limit=1`, or through `gh api 'repos/{owner}/{repo}/actions/workflows/{file}/runs?branch={branch}'`, and confirm a verdict belongs to the commit you mean with `gh api 'repos/{owner}/{repo}/actions/runs?head_sha={sha}'`. `repomatic ci-status` reads the same endpoints and printed one stale table the same way, so a surprising red or green is worth one re-read before acting on it.
