...
- **A test that flips with no code change is an environment diff, not a code one.** Compare what is *installed* against what the lockfile pins before reading a single line of the diff: a venv drifts ahead the moment anything runs a non-frozen sync. `pyproject-fmt` `2.29.2` against a locked `2.28.0` rewrapped one generated array and turned a fixpoint test red, and the same test passed at the same commit once the locked version was reinstalled. That also dates the breakage forward: the lock bump sitting in an open sync pull request is when CI inherits it. The command prunes in the other direction too: `uv sync --group X` sets the group list rather than adding to it, so it removes every other group from a shared checkout, and a plain `uv sync --frozen` afterwards removes those as well. Restore with `uv sync --frozen --all-groups`; reach for `uv run --frozen --with '{pkg}=={X.Y.Z}'` when one tool is missing, since that overlay is ephemeral. A tool in no group at all fails quieter: `uv run` then resolves it from `PATH`, so Homebrew's `ruff` `0.16.8` answered for a `0.16.7` registry pin and the lint result described a version nothing else uses.
- **A slow job is not a hung one, and elapsed time alone cannot tell them apart.** Before calling anything stuck, get a baseline from that workflow's recent *successful* runs (`gh run list --status success`) and read per-job `completedAt` timestamps. A flat count of in-progress jobs is not evidence of a stall: jobs finish and others start into the freed slots, holding the count steady. Diagnosing a hang wrongly is expensive when the proposed remedy is cancelling someone's work.
  ...
  The same holds for a local process, where every *aggregate* signal lies in both directions: a steady process count, a rising highest PID and a non-zero load average are each produced by any work on the machine rather than by the process in question, so a forgotten run from an earlier attempt reads as progress from the one being watched. Walk to the leaf of the process tree instead (`ps -axo pid,ppid,stat,wchan,etime,command`), name what that leaf waits on, and sample it twice. A `portmaster` upgrade held one `make config` PID for 23 minutes on a dialog no terminal could answer, and a second run's forks were read as its progress.

  A surviving PID is *not* by itself evidence of a block: a single long unit of work keeps one PID for many minutes while doing plenty. Read consumed CPU instead, twice (`utime+stime`, fields 14 and 15 of `/proc/<pid>/stat`), and divide by the wall interval. An `eopkg` transaction sat silent for ten minutes on a firmware blob at 78% of one vCPU, and its open descriptors named the package it was extracting; both times the ticks settled it, where elapsed time and log silence had suggested a hang.

  Two probes that lie outright. `kill -0 <pid>` returns EPERM across a privilege boundary, so a `while kill -0` loop watching a **root** process from an unprivileged shell exits immediately and reads as "finished"; use `ps -p <pid>` instead, after checking `ps` answers at all. A sandbox that denies `ps` makes it print nothing, so a watch looping on it finds no process on its first pass and reports the job done before it started: a monitor built on a blocked command reads silence as success. Probe the probe once by hand before trusting a watch built on it. A verification that *counts* findings has the same trap: the pattern must match the format the command printed, or a changed file counts as zero changes and the zero reads as a pass. The tool shell aliases `diff` to `diff --color -ru`, which prints `-` and `+` lines, so `grep -c '^[<>]'` over it finds nothing; `command diff` prints the `<` and `>` form, which `grep -c '^[-+]'` misses. And never poll a package manager's own read-only query for progress: it blocks on the same database lock the transaction holds, so the probe hangs rather than answering.
- **Confirm the machine before debugging its behaviour.** When credentials, config or output disagree with what a host should be doing, the first question is whether it is that host. A DHCP lease moves, an ARP entry goes stale, and a second guest answers on the address the first one held: an hour went into a credential matrix, an MTU probe and an IPv6 test against a machine that was never the target. The SSH host key settles it in one command, and a version banner works too, since the running system's `sshd` should match the one installed. Disabling `StrictHostKeyChecking` for throwaway hosts removes exactly this check, so re-add it by hand when a host misbehaves.
- **An exit code of 0 is not proof of effect.** Verify the file, not the return value. `sudo` over a non-interactive SSH session where a password is required writes nothing and reports nothing useful; `chpasswd` inside a `chroot` exits 0 and leaves `/etc/shadow` untouched, which would have shipped a machine nobody could log into. A shell pipeline that never ran, because a heredoc or nested quoting broke, also succeeds silently. `$?` compounds it: after a pipeline it reports the *last* stage, so `cmd | tail -20; echo $?` prints `tail`'s status and a failed `cmd` reads as success. Capture `${PIPESTATUS[0]}` in bash, `${pipestatus[1]}` in zsh. After any edit that matters, read the target back.

  A backgrounded job repeats the trap one level up, where the reassuring signal is the harness's own. A completion notice read `completed (exit code 0)` for a Python script that had died on an uncaught exception 634 items into 864, and only its output file held the traceback. Read that file before reporting a background job's result, never the completion summary.

  The inverse costs just as much: a probe collapsed to a pass/fail verdict hides why it failed, and some failures are artefacts of the probing. A loop testing five credentials back to back reported the correct one as denied, because the rapid failed attempts before it drew `sshd`'s throttling; run alone with its output shown, the same credential worked. Print what the command actually said rather than reducing it to `WORKS`/`denied`, and space out probes that a remote service may rate-limit.
- **A regex that stops at whitespace mis-parses paths on macOS.** `Mobile Documents`, `Application Support` and most Library paths contain spaces, so a pattern like `file\.filename=([^ ,]+)` truncates mid-path and its match then fails. That reported an attached ISO as absent twice, and sent the diagnosis in the wrong direction both times.
- **Displayed characters are not proof of the bytes.** An exact-match edit missed on a URL holding `2021/03/14`: the literal transcribed from tool output had `-` where the file had `/`. On one output line, `ord()`, `str.count()`, a `unicode_escape` dump and a `replace("/", "[SLASH]")` all reported `/` while the plain rendering of the same string read as `-`, and that plain form did not read the same way across two otherwise identical runs. Take the literal from the file and not from the screen: do the replacement in Python against a string read off disk, or anchor the match on text that holds no ambiguous character. Where one character must be checked, print a control beside it, since a spelled form cannot be misread the way a plain one can. A missed match is the cheap outcome; writing the mis-read literal into a URL or a path is not. Observed 2026-09-17.
- **An escape typed into an edit tool lands decoded.** Tool parameters are JSON, so `\u00a0` written into Python source through the Edit tool arrives as a literal non-breaking space, invisible in any diff. Write the escape with a doubled backslash, or insert it from a script, then scan the file for the raw character before committing. Observed 2026-09-17. Literal blank look-alikes flatten the same way: an nbsp, braille blank or figure space typed into a `write` or `bash` parameter arrived as U+0020, four probe lines testing one space. Build such characters with `chr()` and prove the bytes with `hexdump` before trusting the artifact. Observed 2026-09-23.
...
- **Simplify before adding.** When improving something, first check whether existing code or tools cover the case; remove dead code and unused abstractions before introducing new ones. In this ecosystem the first place to look is the dependency that owns the domain: `extra-platforms` for any platform, architecture, shell or terminal fact, `click-extra` for any CLI plumbing. A hand-rolled basename check for a shell family was a review comment away from `C_SHELLS`, which the declared floor already shipped. A primitive the consumer lacks belongs upstream, as edits in the sibling checkout, not in the consumer.
- **A recorded decision is evidence, not a stop sign.** A note in a rules file saying something was "assessed and rejected" or "kept by decision" describes the code as it stood when someone wrote it. Later edits can shrink what it covers while leaving the sentence intact, and a rationale written about a whole subsystem often only ever justified part of it. Before declining a change on the strength of such a note, restate what it actually decided, check that scope against the code today, and split the verdict when the two have drifted. Then rewrite the note to state the rule the code now follows, since a paragraph defending a subsystem uniformly is what preserves the half that stopped earning it. A verdict pronounced minutes ago in the same session earns no more trust: "too fragile to be worth it" is a claim about an extension point, so name the one you looked for and what it lacked. Declaring the `deprecated` theme slot unreachable cost a whole exchange, when the library exposed a keyword registry the project was already calling three lines away.
...
- **A scripted edit to one member anchors on that member, never on the first match in the file.** A docstring block is found by its command line, and the same prefix recurs across members: `npm.py` documents `npm --global … search --json` under three of them. A `next(...)` over the whole file then rewrites the wrong block, and the suite can stay green because the block it broke was an illustration nothing replays. Locate the `def {name}` line first, search forward from there, and assert something unique to the target block before writing.
...
- **A grep context window is not evidence of drift.** `-A`/`-B` can cut the governing clause mid-sentence, and the amputated text then reads as a stale claim: a docstring correctly recording an upstream closure was flagged stale because the window ended right before its "was closed pointing at…" clause. Read the full block before reporting text as outdated, and again before editing what the flag proposed.
...
- **Two engines select that text, and only one is the terminal's.** Inside the Claude Code TUI, Claude Code holds the mouse: mouse mode is `full` unless `CLAUDE_CODE_DISABLE_MOUSE` or `CLAUDE_CODE_DISABLE_MOUSE_CLICKS` turns it off. Its word rules keep `/` and `.` inside a word and end it at `:`. One double-click on `/Users/kde/code/click/src/click/core.py:107-112` therefore takes the path without the line suffix, and copies it to the clipboard on its own. Keep writing the suffix: it costs the copy nothing.
- **Everywhere else the terminal decides, and it does worse.** At a shell prompt, in the scrollback after a session exits, and in piped output, Apple Terminal `470.2` ends a double-click at `/` as well as `:`, so it returns one path segment (`dotfiles`), one host (`github.com`) or the bare scheme (`https`). Triple-click selects a whole line under both engines, so an address I am expected to paste goes alone on its own line. Measured 2026-09-02.
- **Never let punctuation touch the last character.** A comma or closing bracket against the end joins the selection and corrupts the paste. A period is worse, because both engines count it as part of a word and copy it with the address. End the sentence on the address, or leave a space before the punctuation mark.
...
- **Line-range permalinks pin the full commit SHA, never a moving branch.** `blob/main#L10-L20` silently points at different code after the next push. A tag ref stays true too, but the 40-character SHA is the form that renders as an embedded snippet, and GitHub renders one [only in the repository the link points at](https://docs.github.com/en/get-started/writing-on-github/working-with-advanced-formatting/creating-a-permanent-link-to-a-code-snippet): quote a cross-repo excerpt in a fenced block instead. Read the line numbers out of that commit's blob, since a local checkout ahead of the tag shifts them. A "here is the thing" link without a line range may still use the default branch.
...
- **A release announcement is not a first-release date.** Release notes list what is worth reading about now, not what shipped now, so a headline feature is routinely several releases old. Date a behavior by finding the commit that added it and running `git tag --contains {sha} | sort -V | head -1`. This decides real questions: whether a wrapper's version floor has to move, and what a user one minor behind actually sees.
...
- **Observations in, attribution out.** Present what varied, where, and the link to it; leave the diagnosis and the next step to the maintainer. A maintainer who wrote the code reaches the conclusion faster than any summary of it, and trusts their own reasoning more. This covers the implications too, not the diagnosis alone: a paragraph spelling out what the facts mean for CI, for users, or for the fix they already proposed is padding to a reader technical enough to be maintaining the thing. State the observation and the version it happened on, then stop.
- **A reproducer carries a control.** A failing case proves nothing until a near-identical passing case runs beside it: without one, red cannot be told apart from a broken harness, a missing dependency, or a mistake in the reproducer. Assert the control as well, and have the job report that the test is void, rather than reporting the bug, when the control breaks.
- **Re-read the draft before handing anyone steps to reproduce.** A report that took real work to write already records which paths were tried and which of them survived, and that section is the first thing to go stale in my own head. On 2026-09-16 I handed over a two-stage recipe whose second stage was a crash from `removeAll()` with a submenu open, while the draft on disk listed that exact path among the ones that had survived, and a six-minute QA session went into it. Give the steps the draft calls reproducible, and where a symptom is recorded as a one-off, say so in the same breath rather than letting the steps imply otherwise.
...
- **Never shop for a proxy submitter.** Several distributions refuse a package submitted by the software's own author (Void states it in `CONTRIBUTING.md` § Package Requirements). Where that rule applies, accept it: publish the verified templates and say a distribution user is welcome to submit them *as their own*, with their name in the maintainer field. Asking a named contributor to re-file the work, and above all offering to take maintainership back once it lands, reads as laundering the rule rather than following it.
...
`git show {rev}:{path}` resolves `{path}` from the **repository root**, even under `git -C {subdir}`, where the pathspec form `git log -- {path}` resolves relative to that subdirectory. The two disagree in any repo whose interesting files sit below the top: `git -C /opt/homebrew/Library/Homebrew show 6.0.0:env_config.rb` exits `128` with `fatal: path 'Library/Homebrew/env_config.rb' exists, but not 'env_config.rb'`, plus a hint naming both working spellings. That whole complaint goes to stderr and stdout stays empty, so discarding stderr turns the error into a clean no-match at the next `grep`, and a wrong path then reads as "the code did not exist at that tag".
...
The tool shell is zsh with `nomatch` on: quote every glob meant for the program (`--include='*.swift'`), or an unmatched pattern aborts the whole command with `no matches found` before it runs. An argument starting with `=` dies the same way (`echo ======` fails as `===== not found`), because zsh expands `=name` to the path of a command.

`ruff` anchors its configuration on the current directory when the target project defines none, so running it across repositories applies the wrong rules. `meta-package-manager` sets no `[tool.ruff]` at all, and checking its sources from `~/code/dotfiles` therefore inherited this repository's `fix = true` and isort settings: a bare `ruff check` rewrote a first-party import block its own CI leaves alone. `--show-settings` names the configuration actually in force, and its `cache_dir` gives away which repository won. Pass `--no-fix` whenever the point is to read findings rather than change files. To tell a new violation from one already on `main`, put both sides through one invocation rather than trusting a memory of the before-state: `git show HEAD:{path} | ruff check --no-fix --stdin-filename {path} -`, then the same command fed the working copy. Equal counts prove the change added nothing, whichever configuration is in force.

A repository consuming repomatic that sets no `[tool.ruff]` of its own takes its CI baseline from the sibling checkout, so pass `--config ~/code/repomatic/repomatic/data/ruff.toml`: a system `ruff` of another version selects different rules and formats differently (`0.16.8` unhugged brackets the pinned `0.16.7` keeps), so its `format --diff` reports drift no job would touch.
...
`gh run list` does not answer "what ran last", in **either** of its forms. The branch-wide form returned runs several weeks old at the top of the list, repeatedly; so did `--workflow={file} --branch={branch}`, which put runs from five weeks earlier first while that workflow had in fact run on every recent commit. That reads as "this workflow has not run in a month" and invents a red inventory out of nothing. Read runs through the API instead: `gh api 'repos/{owner}/{repo}/actions/runs?head_sha={sha}'` answers "what ran on this commit", and `gh api 'repos/{owner}/{repo}/actions/workflows/{file}/runs?branch={branch}'` answers "what ran on this branch". Trust the second less: on 2026-09-17 it put runs twelve days old at the top with none of the day's runs in view, so `head_sha` is the form to reach for. Give `head_sha` the **full** 40-character SHA: an abbreviated one matches nothing and returns an empty list, indistinguishable from "nothing ran". `repomatic ci-status` reads the same endpoints and printed one stale table the same way, so a surprising red or green is worth one re-read before acting on it.

`gh search prs` and `gh search issues` return the best-match head of one result set, so `--limit` is a silent truncation: an author-wide sweep capped at 200 came back filled with own-repo noise and dropped the older external items entirely, which reads as "never contributed there". Put every exclusion in the query itself as a raw qualifier after the `--` separator (`gh search prs --author {user} -- -user:{user}`), never in a post-filter; and place all flags before that separator, since anything after it is read as a search term and silently matches nothing. Raise `--limit` toward its 1000 ceiling, and treat a result count equal to the limit as truncated rather than complete.
...
- **A scratch project under `/tmp` fails every path-relative test.** `uv run --project /tmp/{dir}` records the editable install against `/tmp/{dir}` while Python resolves module `__file__` through the symlink to `/private/tmp/{dir}`, so any `Path.relative_to(PROJECT_ROOT)` raises `ValueError: '/private/tmp/…' is not in the subpath of '/tmp/…'` and the suite goes red wholesale (278 of 956 in `meta-package-manager`, every one the same error). Spell the path `/private/tmp/{dir}` everywhere, and delete `.venv` before re-running: the spelling is baked into the existing venv's finder, so passing the resolved path afterwards changes nothing. `PYTHONPATH={scratch}` does not redirect an import either, because an editable install registers a meta-path finder that runs before `sys.path`.
...
- **`git config diff.noprefix` is `true` here, and it leaks into generated artifacts.** Every `git diff` on this machine prints `diff --git basket.txt basket.new.txt` where a default install prints `a/basket.txt b/basket.new.txt`. Any committed file produced by shelling out to git therefore encodes this setting, and CI regenerates it differently: click-extra's `git-diff-screen.svg` capture and the copy its own documentation site published had disagreed on every commit, and that surfaced as one intermittent test failure rather than as a constant one. When a generated artifact runs `git`, pass the flag that pins the rendering (`--no-prefix`, or `--default-prefix` for the other form) instead of inheriting the config, and verify by rebuilding under `GIT_CONFIG_GLOBAL` and `GIT_CONFIG_SYSTEM` pointed at an empty file. It binds the consumption side too: a patch this machine's `git diff` produced needs `git apply -p0`, and without it the apply fails with `does not exist in index` rather than anything naming the prefix.
...
- **File discovery must use `-iname` on this machine.** APFS resolves paths case-insensitively, but `find -name` matching stays case-sensitive, so the lowercase `claude.md` files these repos store are invisible to `find -name CLAUDE.md`. The `ls`, `stat` and `readlink` commands also run through shims: the first two reject their BSD flags, and `readlink` prints nothing for a regular file or for several paths at once; reach for Python when timestamps or link targets matter. The `ls` shim also draws a nerd-font glyph before every entry, so an anchored `ls dir | grep -E '^pytest$'` matches nothing and reads as a missing file: it reported a healthy `.venv` as pruned and cost a killed test run.
- **The `code` CLI reports success without opening anything.** Under the sandbox, `code {file}` exits 0 while the running VS Code never receives the file: the wrapper script backgrounds the launch and never learns it was denied. `ps` is blocked in the same sandbox, which is the tell. Open editors with `open -a "Visual Studio Code" {file}` and the sandbox off, then confirm the app's PID.
- **A Secretive signing refusal is transient.** A `git commit` failing with `Couldn't sign message (signer): agent refused operation?` or `communication with agent failed?` means the Secure Enclave approval prompt went unanswered, not that signing is broken: retry the same commit, twice if needed. A commit that hangs to the tool timeout instead is the same unanswered prompt. A different message names the sandbox instead: `cannot exec '/Users/kde/.ssh/ssh-keygen-secretive': Operation not permitted` needs the sandbox off, and no number of retries helps.
- **A scratch git repo inherits the global `commit.gpgsign`.** A `git commit` in a throwaway checkout hangs on the same Secretive approval as any signed commit, and a script that commits scratch scenes in a loop waits there until its timeout: pass `-c commit.gpgsign=false` to every commit in a repo nothing will ever push.
