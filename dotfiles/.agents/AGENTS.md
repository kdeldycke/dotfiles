...
- **File discovery must use `-iname` on this machine.** APFS resolves paths case-insensitively, but `find -name` matching stays case-sensitive, so the lowercase `claude.md` files these repos store are invisible to `find -name CLAUDE.md`. The `ls` and `stat` commands also run through shims that reject their BSD flags; reach for Python when timestamps matter. The `ls` shim also draws a nerd-font glyph before every entry, so an anchored `ls dir | grep -E '^pytest$'` matches nothing and reads as a missing file: it reported a healthy `.venv` as pruned and cost a killed test run.
